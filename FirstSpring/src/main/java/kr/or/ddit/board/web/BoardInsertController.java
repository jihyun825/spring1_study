...
public class BoardInsertController {
	...
	public String boardInsert(BoardVO boardVo,Model model) {
		...
		//org.apache.commons라이브러리를 이용하여 boardVo안에 들어있는 제목,내용을 공백/null체킹을 통해 입력값 필터를 처리할 수 잇다.
		...
		if(errors.size()>0) {
			//model은 데이터 전달자
			// 내가 넘기고자하는 데이터를 대신 넘겨주는 역할을한다.
			model.addAttribute("errors",errors);
			model.addAttribute("board",boardVo);
			goPage = "board/form";//에러가발생
		}else {//에러가 발생하지않고 정상적인 데이터가 발생
