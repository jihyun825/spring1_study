...
public class PaginationInfoVO<T> {
	...
	private String searchType;			// 검색타입(제목,내용,작성자,작성일 등등)
	private String searchWord;			// 검색단어(키워드)
	
	public PaginationInfoVO() {
	
	}
	//pagenationInfoVO 객체를 만들때, 한 페이지당 게시글 수와 페이지 블록 수를 원하는 값으로 초기화 할 수 있도록 custom생성자 생성
	...
	public void setCurrentPage(int currentPage) {
		...
		endRow = currentPage * screenSize;// 끝row = 현재페이지 * 한페이지당 게시글 수
		
		startRow = endRow - (screenSize -1); // 시작row = 끝row -(한페이지당 글수 -1)
	...
	public String getPagingHTML() {
		...
		// 부트스트랩을 이용하던 여러css요소를 이용해서 페이징 UI를 구현하는 곳입니다.
